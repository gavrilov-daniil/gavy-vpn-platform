...
- **node-agent v1 — тонкий:** запись полного конфига в файл + `systemctl restart` Xray. Без live-`AlterInbound`-дельты и gRPC. Чтение статистики с Xray тоже ещё не реализовано (`xray.Stats()` — заглушка), поэтому буфер обычно пуст.
